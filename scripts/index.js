  ...
  await axios
    ...
    .then(async (res) => {
      if (res.data.err) {
        ...
        inTreatment = false;
        alert(
          "Usuário já está sendo atendido! Sempre atualize seus tickets antes de chamar"
        );
        autoReload();
        return;
      }
      document.getElementById(
        "finish"
      ).innerText = `Finalizar atendimento ${res.data.response_data.value}`;
      ...
      document.getElementById("infoTicket").innerHTML = `
                ...
                <h3>Preferencial: ${
                  res.data.response_data.isPreferred ? "Sim" : "Não"
                }</h3>
      ...
      sequencePreferred =
        (await sequencePreferred) == 2 ? 0 : sequencePreferred + 1;
    })
    .catch(async (err) => {
      if (err?.status == 401) {
        return (window.location.href = "login.html");
...
async function finishTreatment(dataTicketOld) {
  ...
  if (dataTicketOld && dataTicketOld[3]) {
    if (
      !window.confirm(
        `Tem certeza que deseja finalizar o Ticket ${dataTicketOld[0]} que já foi chamado ? `
      )
    ) {
      return;
    }
    ...
    .post(
      "http://172.16.254.253:6100/treatment/finish",
      {
        ...
        sector:
          dataTicketOld && dataTicketOld[3]
            ? dataTicketOld[2]
            : selectedTicketNumber[2],
        isPreferred:
          dataTicketOld && dataTicketOld[3]
            ? dataTicketOld[4]
            : selectedTicketNumber[3],
      },
      {
        headers: {
          "Access-Control-Request-Private-Network": true,
...
async function autoReload() {
  ...
  await axios
    ...
    .then(async (res) => {
      ...
      var ticketsTempData = res.data.response_data.filter(
        (ticket) =>
          ticket.isFinished === false &&
          ticket.isWaiting === true &&
          ticket.sectorName == JSON.parse(userInfosCookie).sector
      );
      var tempDataNotFinished = res.data.response_data.filter(
        (ticket) =>
          ticket.isFinished === false &&
      ...
      if (JSON.stringify(tickets[0]) != JSON.stringify(ticketsTempData[0])) {
        ...
        JSON.stringify(tickets[0]) == JSON.stringify(ticketsTempData[0]) &&
        ticketsTempData.length > 0 &&
        !inTreatment
      ) {
...
async function createTicketDivs() {
  ...
  if (!tickets || tickets.length == 0) {
    ...
    // chama 2 Preferenciais, depois chama dois comuns
    ...
    const preferentialTextOne = tickets[0].isPreferred
      ? "Ticket Preferencial*"
      : "Não Preferencial";
    ...
    tickets.forEach((ticket) => {
      ...
      const buttonDiv = document.createElement("div");
      const preferentialText = ticket.isPreferred
        ? "Ticket Preferencial*"
        : "Não Preferencial";
      const ticketNumberTwo = tickets[0].value;
